...
def evaluate(data_name, prompt_type, samples: list=None, file_path: str=None, max_num_samples=None, execute=False, use_glyph=False):
    ...
    if ('pred' not in samples[0]) or execute:
        ...
        for sample in tqdm(samples, desc="Execute"):
            ...
            for code in sample['code']:
                pred, report = run_execute(executor, code, prompt_type, data_name, execute=execute, use_glyph=use_glyph)
    ...
    acc_all = round(100 * np.array(score_mat).mean(),1)
