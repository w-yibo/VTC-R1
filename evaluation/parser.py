...
def extract_answer(pred_str, data_name, use_glyph=False):
    ...
    if use_glyph:
            parts = pred_str.split('</think>')
            if len(parts) > 1:
                # 获取 </think> 之后的内容
                content = str(parts[-1])
                
                # 检查是否存在 <answer> 和 </answer> 标签
                if '<answer>' in content and '</answer>' in content:
                    # 提取两个标签之间的内容：
                    # 1. split('<answer>')[1] 取 <answer> 之后的部分
                    # 2. split('</answer>')[0] 取 </answer> 之前的部分
                    content = content.split('<answer>')[1].split('</answer>')[0]
                
                # 如果提取出的内容不为空，则更新 pred，否则保持原样
                pred = content.strip() if content.strip() != '' else pred
    # multiple line
    # pred = pred.split("\n")[0]
    pred = re.sub(r"\n\s*", "", pred)
    if pred != "" and pred[0] == ":":
...
def parse_ground_truth(example: Dict[str, Any], data_name):
    ...
    # parse ground truth
    if data_name in ["math", "math_oai", "minerva_math", "ocw", "amps", "hungarian_exam", "aime24"]:
        gt_cot = example['solution']
        gt_ans = extract_answer(gt_cot, data_name)
    elif data_name == "aime25" or data_name == "amc23" or data_name == "gpqa_d":
...
def run_execute(executor, result, prompt_type, data_name, execute=False, use_glyph=False):
    ...
    if "program_only" in prompt_type:
        ...
        prediction = extract_answer(result, data_name, use_glyph)
